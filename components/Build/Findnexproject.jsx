...
const Card = (props) => {
  ...
  const handleLearnMore = () => {
    if (user?.user?.name) {
      router.push("/project_categories");
    } else {
      setOpen1(true)
  ...
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      className="w-72 text-center group pt-10 w-full relative mr-12 items-center justify-center"
      id="FindNextProject"
    >
      <div className="bg-clip-padding backdrop-filter px-10 py-6 backdrop-blur-xl bg-opacity-60 border border-white/20 to-black bg-gradient-to-tl from-white/10 hover:rotate-1 hover:backdrop-blur-3xl hover:scale-105 group duration-500 rounded-3xl">
        ...
        <div>
          <Dialog open={open1} fullWidth={true} maxWidth="sm">
            ...
            <DialogActions className="bg-black text-black"></DialogActions>
          </Dialog>
        </div>
      </div>
...
const data = [
  {
    ...
    imgLink: biginnerImg,
    content:
      "find a beginner-level project for you. Which requires entry-level skills and a short time to complete.",
  },
  {
    title: "Intermediate",
    imgLink: intermediateImg,
    content:
      "Work on mid-level skill projects with complex problem-solving to advance your skills.",
  },
  {
    title: "Expert",
    imgLink: expertImg,
    content:
      "It's a whole team, multi-domain project where you solve real life problems and develop an MVP.",
  },
];
    ...
    <div className="mt-5" id="project">
      ...
      >
        <div className={styles.w__card__holder}>
          ...
          <div className="ml-6 md:md-0 grid grid-cols-1 md:grid-cols-3 gap-20">
          {data.map((item, index) => {
            return (
              <Card
                key={index}

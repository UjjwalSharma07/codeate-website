...
function Project({
  ...
  return (
    ...
    <motion.div
     ...
     className="bg-clip-padding w-[500px] h-[600px] flex flex-col mt-2 justify-center items-center backdrop-filter px-6 py-4 backdrop-blur-xl bg-opacity-60 border border-white/10 bg-gradient-to-tl to-amber-800/10 group duration-500 rounded-3xl card-animation hover:border-black/80 via-slate-900 from-black/10"
   >
    {imageuri? (
    <Image  src={imageuri}
      alt="img" 
  ...
  <div className=" flex-row justify-center items-center">
      <Link href={`projects/${id}`}>
        <button onClick={()=>{setId(id)}} className="px-4 py-2 duration-300 font-medium text-white bg-blue-500 rounded-full hover:bg-blue-700 focus:outline-none focus:shadow-outline mx-2 mb-10">
    ...
    <Link href="https://discord.com/invite/v4vZBMt9hQ" target="_blank">
      <button
        ...
        className="px-4 mr-5 py-2 font-medium text-sm text-white bg-gray-500 rounded-full hover:bg-gray-700 focus:outline-none focus:shadow-outline"
      >
      ...
      <button
        onClick={onDiscussClick}
        className="px-4 py-2 font-medium text-sm text-white bg-gray-500 rounded-full hover:bg-gray-700 focus:outline-none focus:shadow-outline"
      >
